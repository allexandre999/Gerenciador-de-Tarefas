...
async function handleCredentialResponse(response) {
    try {
        const idToken = response.credential; // Obtém o token de autenticação do Google
        console.log("Token de autenticação:", idToken);

        // Envia o token para o backend (Parse Server) para autenticação
        const user = await Parse.Cloud.run('googleLogin', { idToken });
        console.log("Resposta do backend:", user);

        // Armazena o idToken no sessionStorage
        localStorage.setItem('googleIdToken', idToken);
        console.log("idToken armazenado:", idToken);

        if (user) {
            alert("Login com Google realizado com sucesso!");

            // Obtém o access_token via OAuth 2.0
            const accessToken = await authenticateGoogle();
            console.log("Access Token obtido:", accessToken);

            window.location.href = 'tarefa.html'; // Redireciona para a página de tarefas
        }else {
            throw new Error("Falha ao autenticar no Parse Server.");
        }
    } catch (error) {
...
async function authenticateGoogle() {
    return new Promise((resolve, reject) => {
        google.accounts.oauth2.initTokenClient({
            ...
            scope: 'https://www.googleapis.com/auth/calendar.events https://www.googleapis.com/auth/calendar', // Escopos necessários
            redirect_uri: 'http://127.0.0.1:5502',
            callback: (response) => {
                if (response.error) {
                    console.error("Erro na autenticação OAuth:", response);
                    reject(response.error);
                } else {
                    console.log("Access Token recebido:", response.access_token);
                    localStorage.setItem("googleAccessToken", response.access_token); // Armazena o access_token
